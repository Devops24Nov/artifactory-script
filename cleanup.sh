...
RESULTS=`curl -s -X GET -u admin:admin "http://13.235.42.32:8081/artifactory/api/search/creation?from=1325376000000&to=1567881000000&repos=example-repo-local" | grep uri | awk '{print $3}' | sed s'/.$//' | sed s'/.$//' | sed -r 's/^.{1}//'`
for RESULT in $RESULTS ; do
    ...
    PATH_TO_FILE=`curl -s -X GET -u admin:admin $RESULT | grep downloadUri | awk '{print $3}' | sed s'/.$//' | sed s'/.$//' | sed -r 's/^.{1}//'`
    echo "deleting path $PATH_TO_FILE"
    # deleting the corresponding artifact at last
    curl -X DELETE -u admin:admin $PATH_TO_FILE
done
